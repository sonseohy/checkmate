...
export default function Dashboard() {
    ...
    return (
        <div> 
            {/* 활동에 대한 그래프 */}
            <div className={`flex mt-10 ${isMobile ? 'flex-col justify-center' : 'flex-row gap-10 ml-10'} items-center `}>
                <div className={` rounded-2xl bg-white shadow-[0_0px_15px_rgba(0,0,0,0.2)] ${isMobile ? 'w-full mb-5' : 'w-100'}`}>
                    <div className="mt-5 text-2xl ml-5 font-semibold">
                        계약 활동
                    </div>
                    <PieDonutChart />
                </div>
                <div className={`flex-1 w-full h-[450px] rounded-2xl bg-white shadow-[0_0px_10px_rgba(0,0,0,0.2)] `}>
                    <div className="mt-5 text-2xl ml-5 font-semibold">
                        최근 계약서
                    </div>
